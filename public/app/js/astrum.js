...
var ndplComponent = Vue.extend({
    ...
    inline_styles: function() {
      ...
      if (_this.loaded) {
        if (_this.component.options.sample_min_height) {
          styles +=
            'min-height:' + _this.component.options.sample_min_height + 'px;'
        }
      }
      ...
      if (_this.component.options.sample_background_color) {
        styles +=
          'background-color:' +
          _this.component.options.sample_background_color +
          ' !important;'
      }
  ...
  methods: {
    ...
    updateActive: function() {
      ...
      if (
        _this.$root &&
        _this.$root.scroll_position >= _this.$el.offsetTop - 60 &&
        _this.$root.scroll_position <
          _this.$el.offsetTop + _this.$el.offsetHeight
      ) {
      ...
      setTimeout(function() {
        if (_this.$el.querySelector('.ndpl-component__code')) {
          /**
                     ...
                     */
          if (
            _this.$el.querySelector('.ndpl-component__sample').offsetHeight <=
              74 &&
          ...
          if (
            _this.component.options.disabled_auto_sample_hiding &&
            _this.component.options.disabled_auto_sample_hiding.hasOwnProperty(
              'show_on_mobile'
            ) &&
            _this.component.options.disabled_auto_sample_hiding.hasOwnProperty(
              'show_on_desktop'
            ...
            if (_this.$root.mobile_view) {
              _this.hide_sample_code = !_this.component.options
                .disabled_auto_sample_hiding.show_on_mobile
            } else {
              _this.hide_sample_code = !_this.component.options
...
new Vue({
    ...
    all_creators: function() {
      ...
      if (this.creators.length && this.creators[0].name) {
        for (var i = 0; i < this.creators.length; i++) {
          ...
          formattedCreators +=
            prefix + '<a href="' + url + '" target="_blank">' + name + '</a>'
        }
  ...
  ready: function() {
    ...
    _this.mobile_view =
      _this.window_outer_width >= _this.breakpoint ? false : true
  ...
  methods: {
        ...
        .then(function(response) {
          _this.initData(response.data, function() {
            if (_this.$data.groups.length) {
              ...
              _this.logInfo(
                'You need to add a component to your library before it can be loaded.<br/>You can either do this manually by editing your <code>data.json</code> file,<br/> or you can use the command line helper: <code>astrum new [group_name/component_name]</code>'
              )
            }
          })
        })
    ...
    setupComponents: function() {
      ...
      for (var i = 0; i < _this.groups.length; i++) {
        ...
        for (var j = 0; j < group.components.length; j++) {
          // Set default variables
          _this.$set(
            'groups[' + i + '].components[' + j + '].id',
            'group-' + group.name + '-component-' + group.components[j].name
          )
          _this.$set(
            'groups[' + i + '].components[' + j + '].group_id',
            'group-' + group.name
          )
          _this.$set('groups[' + i + '].components[' + j + '].active', false)
          _this.$set(
            'groups[' + i + '].components[' + j + '].options',
            group.components[j].options ? group.components[j].options : false
          )
          _this.$set(
            'groups[' +
              i +
              '].components[' +
              j +
              '].options.sample_always_show',
            group.components[j].options.sample_always_show
              ? group.components[j].options.sample_always_show
              : false
          )
          _this.$set(
            'groups[' +
              ...
              '].components[' +
              j +
              '].options.sample_mobile_hidden',
            group.components[j].options.sample_mobile_hidden
              ? group.components[j].options.sample_mobile_hidden
              : false
          )
          _this.$set(
            'groups[' +
              ...
              '].components[' +
              j +
              '].options.sample_dark_background',
            group.components[j].options.sample_dark_background
              ? group.components[j].options.sample_dark_background
              : false
          )
          _this.$set(
            'groups[' +
              ...
              '].components[' +
              j +
              '].options.disable_code_sample',
            group.components[j].options.disable_code_sample
              ? group.components[j].options.disable_code_sample
              : false
          )
          _this.$set('groups[' + i + '].components[' + j + '].code_show', false)
          _this.$set(
            'groups[' + i + '].components[' + j + '].type',
            group.components[j].type ? group.components[j].type : 'standard'
          )
          _this.$set(
            'groups[' + i + '].components[' + j + '].width',
            group.components[j].width ? group.components[j].width : 'full'
          )
    ...
    loadGroup: function(group) {
      ...
      _this.$http
        .get(group_path + '/description.md' + '?cb=' + new Date())
        .then(
          function(response) {
          ...
          function() {
            _this.logError(
              'Description file for <strong>' +
                group.name +
                '</strong> group failed to load from <code>' +
                group_path +
                '/description.md</code>'
            )
          }
    ...
    loadComponent: function(component) {
      ...
      _this.$http
        .get(component_path + '/markup.html' + '?cb=' + new Date())
        .then(
          function(response) {
          ...
          function() {
            _this.logError(
              'HTML file for <strong>' +
                component.name +
                '</strong> component failed to load from <code>' +
                component_path +
                '/html.md</code>'
            )
          }
      ...
      // Get and set component description
      _this.$http
        .get(component_path + '/description.md' + '?cb=' + new Date())
        .then(
          function(response) {
